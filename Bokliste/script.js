...
var bookController = (function() {
	...
	var Book = function(title, release, author, pages, level, score, registered, comment, image, link, count) {
	...
	return {
		...
		addBook: function(titl, rel, auth, pg, lvl, scr, reg, com, img, lnk, cnt) {
			...
			newItem = new Book(titl, rel, auth, pg, lvl, scr, reg, com, img, lnk, cnt);
...
var UIcontroller = (function() {
	...
	return {
		...
		displayData: function(elm, cont) {
			...
			return {
					title: elm.querySelector(DOMstrings.bookTitle).textContent = cont.title,
					release: elm.querySelector(DOMstrings.bookRelease).textContent = cont.release,
					author: elm.querySelector(DOMstrings.bookAuthor).textContent = cont.author,
					pages: elm.querySelector(DOMstrings.bookPages).textContent = cont.pages,
					level: elm.querySelector(DOMstrings.bookLevel).textContent = cont.level,
					score: elm.querySelector(DOMstrings.bookScore).textContent = cont.score,
					registered: elm.querySelector(DOMstrings.bookRegistered).textContent = cont.registered,
					comment: elm.querySelector(DOMstrings.bookComment).innerHTML = cont.comment,
...
var controller = (function(bookController, UIcontroller) {
	...
	function makeDOMList() {
		...
		for (i = 0; i < numberOfBooks; i++) {
			...
			bookController.addBook(input.title, input.release, input.author, input.pages, input.level, input.score, input.registered, input.comment, input.image, input.link);
			...
			bookList[i].addEventListener("click", function(evt) {
				...
				if (target.className === 'btn__comment') {
			    	...
			    	this.querySelector(DOM.commentsWrapper).classList.remove("close_comment");
			    	this.querySelector(DOM.commentsWrapper).classList.add("open_comment");

			    // If the button is "close comment" we hide the comment field.
			  	} else if (target.className === 'btn__close_comment') {
			  		
			  		this.querySelector(DOM.commentsWrapper).classList.remove("open_comment");
			  		this.querySelector(DOM.commentsWrapper).classList.add("close_comment");
			  }
			}, false);
		...
		document.querySelector(DOM.showMore_btn).innerHTML = "Show more " + "(" + remainingItems + ")";
	...
	var setupEventListeners = function() {
		...
		document.querySelector(DOM.categorySelector).addEventListener("change", selectCategory);

		// Define the sorting order selector.
		document.querySelector(DOM.orderSelector).addEventListener("change", changeOrder);

		// Define the load more button.
		document.querySelector(DOM.showMore_btn).addEventListener("click", loadMoreBooks);
		
	};



	// Function to make us define what property want to sort on.
